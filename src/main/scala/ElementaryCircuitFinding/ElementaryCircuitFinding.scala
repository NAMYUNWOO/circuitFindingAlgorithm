object ElementaryCircuitFinding {
  ...
  def stringEdgeToPairEdgeWithDelemeterAttr(edgeList:Seq[String],delemeter:String):Seq[Seq[String]] = {
    def go(rest:Seq[String],acc:List[List[String]]):List[List[String]] = rest match {
      case Seq(head:String, tail @ _*)=> go(tail,head.split(delemeter).toList::acc)
      case Seq() => acc
      case _ => acc
    }
    go(edgeList,Nil)
  ...
  def stringEdgeToPairEdgeWithArrowDelemeter(edgeList:Seq[String]):Seq[Seq[String]] = {
    ...
    def go(rest:Seq[String],acc:List[List[String]]):List[List[String]] = rest match {
      case Seq(head:String, tail @ _*)=> go(tail,head.split(arrowDelemeter).toList::acc)
      case Seq() => acc
      case _ => acc
    }
    go(edgeList,Nil)
